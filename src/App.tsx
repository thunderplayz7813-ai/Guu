  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Express backend API unavailable, falling back to direct client-side Gemini stream:", err);
        fetchFailed = true;
      }

      // If backend call failed (e.g. on Netlify static hosting), run direct client-side inference!
      if (fetchFailed) {
        const clientApiKey = customApiKey || (import.meta.env.VITE_GEMINI_API_KEY as string) || "";
        if (!clientApiKey) {
          throw new Error("Sura AI is running in Internet-Only Mode without a backend proxy (such as on a Netlify static build). Please configure your own Custom Gemini API Key in the Settings panel (gear icon in the top right) or set VITE_GEMINI_API_KEY in your deployment environment variables to stream directly from the model.");
        }
      ...
      console.error("Inference failed completely:", err);
      const displayMsg = err?.message || "Please check your network status and API key configuration.";
      setSessions((prev) =>
        prev.map((s) =>
          s.id === sessionToUpdate!.id
            ? {
                ...
                messages: s.messages.map((m) =>
                  m.id === assistantMsgId
                    ? {
                        ...m,
                        content: `**Inference Failed:** ${displayMsg}`
                      }
                    : m
                )
              }
