...
app.post("/api/chat", async (req, res) => {
  ...
  if (!requestApiKey) {
    ...
    res.write(`data: ${JSON.stringify({ error: "No Gemini API Key provided. Sura AI is currently configured in Internet-Only mode. Please set up GEMINI_API_KEY in your server environment or enter your custom key in the Settings panel." })}\n\n`);
    res.write("data: [DONE]\n\n");
    return res.end();
  }
  ...
  try {
    ...
    if (!stream) {
      const errorMsg = lastError?.message || "Failed to establish a connection to Gemini after multiple retries. Please check your internet connection and API Key status.";
      res.write(`data: ${JSON.stringify({ error: `Sura connection error: ${errorMsg}` })}\n\n`);
      res.write("data: [DONE]\n\n");
      return res.end();
    }
    ...
    const errMsg = error?.message || "Internal server disruption detected.";
    if (!res.headersSent) {
      res.setHeader("Content-Type", "text/event-stream");
      res.setHeader("Cache-Control", "no-cache");
      res.setHeader("Connection", "keep-alive");
